...
The emitter generates a JSON Schema (Draft 2020-12) document from a Zig type at
comptime. It is independent of the [runtime validator](validator.md).
...
- [`$defs` and recursion](#defs-and-recursion)
- [Strict preset](#strict-preset)
- [Typed value validation](#typed-value-validation)
- [Tool schema descriptor](#tool-schema-descriptor)
- [Root object wrapper](#root-object-wrapper)
...
const User = struct {
    ...
    pub const jsonschema = .{
        ...
        .fields = .{
            .name = .{ .description = "The user's full name.", .minLength = 1 },
            .age = .{ .description = "Age in years.", .minimum = 0, .maximum = 130 },
            .email = .{ .format = "email" },
        },
    };
};
...
```json
{
  ...
  "properties": {
    "name": { "type": "string", "description": "The user's full name.", "minLength": 1 },
    "age": { "type": "integer", "description": "Age in years.", "minimum": 0, "maximum": 130, "default": 18 },
    "email": {
...
All struct fields are required by default. Zig field defaults are emitted as JSON
Schema `default`, and the field stays in `required`.
...
`Options.field_naming` can transform Zig field names globally, for example
`.camelCase` or `.PascalCase`. Field metadata `.name` renames one emitted JSON
property and overrides the global naming policy. Use `.@"const"` for JSON Schema
`const`, `.required` to override requiredness, and `.omit` to exclude a field.
Core keywords use escaped field names such as `.@"$id"`, `.@"$anchor"`, and
`.@"$comment"`. Type metadata `.discriminator` sets the discriminator field for
`union(enum)` schemas. Unknown metadata keys, metadata for missing fields,
duplicate emitted field names, invalid metadata types, invalid defaults, and
constraints on the wrong field type are compile errors.

## Maps

String-key maps emit dictionary schemas with `additionalProperties` as the value
schema. Supported map shapes include `std.StringHashMap(T)`,
`std.StringHashMapUnmanaged(T)`, and `std.StringArrayHashMapUnmanaged(T)`.

## Tagged unions

`union(enum)` emits an object schema with `oneOf`. By default it matches Zig's
JSON encoding: an externally tagged object such as `{ "search": { "query": "zig" } }`.

Add type metadata `.discriminator` to emit a flattened discriminated-object
schema instead. Each variant then gets a discriminator field with a string
`const` matching the tag. Struct payload variants are flattened into the variant
object. Scalar and tuple payload variants use a generated `value` field.

`.discriminator` changes the schema/stringify shape. It is not the native
`std.json` union parse shape. Use the default externally tagged union shape when
parsing directly with `std.json`.
...
`use_defs = .auto` is the default. Recursive schemas automatically use `$defs`
and `$ref`. Use `.always` to force `$defs` for nested structs or `.never` to
reject recursion.
...
Use `jsonschema.strict_options` for a provider-neutral strict schema shape: no
top-level `$schema`, no defaults, required fields, closed objects, `$defs` auto
mode, and inferred numeric/array bounds.
...
`validateValue` checks a parsed Zig value against supported metadata constraints
and writes path errors. It is a convenience for emitter metadata, not a full
JSON Schema validator. For that, use the [runtime validator](validator.md).
...
`toolSchemaAlloc` returns a provider-neutral descriptor with name, description,
and schema JSON. Provider request payloads stay outside this package.
...
Some consumers require the root schema to be an object. `root_wrapper` wraps any
schema in an object property. This changes the expected JSON shape; callers must
parse the wrapper object and unwrap the field.
...
The tables below describe which keywords the **emitter** produces and which the
legacy typed-value checker (`validateValue`) asserts. They do **not** describe
the [runtime validator](validator.md), which supports every Draft 2020-12 keyword.
